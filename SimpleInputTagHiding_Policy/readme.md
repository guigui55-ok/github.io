...
本プライバシーポリシーは、utauinu8102（以下、「当開発者」）が開発した Google Chrome の拡張機能 (Extension) (以下、「拡張機能」とします。) の利用において、利用者の個人情報もしくはそれに準ずる情報を取り扱う際に、当開発者が遵守する方針を示したものです。
...
フォームに入力された内容は、ブラウザ内の LocalStorageに保存されます。
...
- 拡張機能で用いるログインパスワード
- 拡張機能上で利用者が入力したキーワード
上記の情報について、ログインパスワードを設定することにより、拡張機能の設定値がシステム管理者以外から容易に変更されないようにします。
また、上述のキーワードを設定することにより、拡張機能の目的の動作が実行されます。
...
メールアドレス：utauiunu8102@nifty.com
